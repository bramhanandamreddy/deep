...
function activate(_) {
	...
	const blackboxAuthTokenKey = 'blackbox.authtoken';
	_.globalState.setKeysForSync([blackboxAuthTokenKey]);

	currentToken = _.globalState.get(blackboxAuthTokenKey);

	const handleUri = async (uri) => {
		const queryParams = new URLSearchParams(uri.query);

		if (queryParams.has('accessToken')) {
			// vscode.window.showInformationMessage(`URI Handler says: ${queryParams.get('accessToken')}`);
			...
			try {
				...
				const repositories = repoResponse.data.map((repo) => repo.full_name);
				_.globalState.update(blackboxAuthTokenKey, accessToken);
				vscode.window.showInformationMessage('You have successfully logged in');
				selectionFct("success connect repo")
				vscode.commands.executeCommand( "blackbox.showChat" )
			} catch (error) {
	...
	_.subscriptions.push(
        vscode.commands.registerCommand('extension.authenticateGitHub', () => {
			const currentToken = _.globalState.get(blackboxAuthTokenKey);
			if(!currentToken){
				vscode.env.openExternal(vscode.Uri.parse(`https://www.blckbx.ai/login`));
			}else{
	...
	if (installed_date){
		if (installed_date > inline_sep1_update && installed_date < inline_nov20_update){
  ...
  vscode.commands.registerCommand('blackbox.terminal.search', async () => {
	...
	let terminalContentFormatted = formatTerminalContent(terminalContent);

    is_triggered_from_Q = true;
	lastSearchedQuery = terminalContentFormatted;
	queryFromOutside = true;
	is_stream = true;

	terminalContentFormatted = '```\n'+ terminalContentFormatted + '\n```'
  ...
  const terminalPanel = vscode.window.registerWebviewViewProvider('terminal.search.view',{
    resolveWebviewView(webviewView) {
      ...
      webviewView.onDidChangeVisibility(() => {
        ...
        vscode.window.activeTerminal.show();
		selectionFct('click top terminal')
      });
    }
  });
  ...
  function formatTerminalContent(content) {
			...
			try {
				...
				thisWebview.webview.onDidReceiveMessage(async (data) => {
					...
					if (data.command) {
						...
						} else if (data.command === "repoConnect") {
							vscode.commands.executeCommand('extension.authenticateGitHub')
							selectionFct("click connect repo")
	...
	const saveFileToServer = async (fileIdentifier, fileContent, filePath, language) => {
		const url = "https://www.useblackbox.io/share-code"
		const headers = {
		...
		try{
			...
			if (result.status == 'success'){
				copyToClipboard(`https://www.useblackbox.io/editor?id=${fileIdentifier}`)
		...
		`
			...
			<body>
				...
				<l class='text-description'>
					...
					2. Blackbox AI Code History<br><br>
					<div class="btn-default btn-diff-open btn-on">
						<span>View Diff History</span>
					</div>
					3. Blackbox AI Repo Chat<br><br>
					<div class="btn-default btn-repo-connect btn-on">
						<span>Chat with Your Repo</span>
					</div>
				</l>
				<br><br>
				...
				<script>
					(function() {
						...
						$(document).on("click", ".btn-repo-connect", function(){
							postMessage({command:"repoConnect"})
						})
	...
	vscode.commands.registerCommand("blackbox.showChat", (queryParam) => {
		...
		if (chatPanel) { 
			...
			let chatUrl = 'https://www.codechat.co'
			
			currentToken = _.globalState.get(blackboxAuthTokenKey);
			if (currentToken && !queryParam){
				// get repo rul
				let repoUrl = getRepoUrl()
				if (repoUrl){
					chatUrl += `/?access_token=${currentToken}&repo=${repoUrl}`	
				}
			}
  ...
  <body style="margin:0px;padding:0px;overflow:hidden">
	...
	<script>
        ...
        const sendMessageToChild = (message) => {
			...
			chatPanel.webview.onDidReceiveMessage(
				(message) => {
					if (message.command === "request-active-editor-code") {
						let text = getSelectedTextOrVisible();
						let message = {
					...
					else if (message.command === 'toggle-repo-search'){
						currentToken = _.globalState.get(blackboxAuthTokenKey);
						if (!currentToken){
							open('https://www.blckbx.ai/login')
						}
						if (getRepoUrl() === null){
							vscode.window.showInformationMessage('Open a Repo to Activate Repo Search');
						}
					}
				},
				undefined,
	...
	const saveToCloud = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 1);
	//icon
	saveToCloud.text= 'Share Code Link';
	saveToCloud.tooltip = 'Share Code Link';
	saveToCloud.command = 'extension.saveCodeToCloud';
	saveToCloud.show();

	// Commmit Message Status Bar
	let isGitActive = gitActive()
	if (isGitActive){
		const makeCommitMessage = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 1);
		//icon
	...
	const explainCodeStatusBar = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 1);
	...
	const statusBarCodeChat = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 1);
	//icon
	...
	vscode.commands.registerCommand("blackbox.commentFile", async () => {
        ...
        if (userInput) {
            // You can process the input here
				...
				let content = '```'+ editor.document.languageId+ '\n' + fileContent + '\n```\n' + instruction
				vscode.commands.executeCommand("blackbox.showChat", content);
...
const getRepoUrl = ()=>{
	...
	try{
		const currentWorkspaceFolder = vscode.workspace.workspaceFolders[0].uri.fsPath;
		try{
			repo = cp.execSync("git remote get-url origin", { cwd: currentWorkspaceFolder }).toString();
		}catch(e) {
			console.log('Error', e)
		}
	}catch(e){
